...
export const GetAllPermissions = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(200).json(new ApiResponse(200, { permissions, pages, total }, 'Permissions retrieved successfully'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
...
export const GetPermissionsActionsList = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(200).json(new ApiResponse(200, actionList, 'Action list retrieved successfully'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
...
export const SoftDeletePermission = asyncHandler(async (req, res, next) => {
  const { id } = req.params
  try {
    const permission = await Permission.findByIdAndUpdate(id, { status: 'inactive', isDeleted: true }, { new: true })
    if (!permission) {return next(new ApiError(404, 'Permission not found'))}
...
export const RestorePermission = asyncHandler(async (req, res, next) => {
  const { id } = req.params
  try {
    const permission = await Permission.findByIdAndUpdate(id, { status: 'active', isDeleted: false }, { new: true })
    if (!permission) {return next(new ApiError(404, 'Permission not found'))}
    res.status(200).json(new ApiResponse(200, null, 'Permission restored successfully'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
...
export const PermanentDeletePermission = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!permission) {return next(new ApiError(404, 'Permission not found'))}
    res.status(200).json(new ApiResponse(200, null, 'Permission deleted permanently'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
