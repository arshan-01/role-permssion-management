...
export const GetAllRoles = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(200).json(new ApiResponse(200, { roles, total, pages }, 'Roles retrieved successfully'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
...
export const UpdateRole = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // Update role
    const role = await Role.findByIdAndUpdate(id, { name, permissions, status }, { new: true })
    if (!role) {return next(new ApiError(404, 'Role not found'))}
...
export const RestoreRole = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!role) {return next(new ApiError(404, 'Role not found'))}
    res.status(200).json(new ApiResponse(200, null, 'Role restored successfully'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
...
export const PermanentDeleteRoleController = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!role) {return next(new ApiError(404, 'Role not found'))}
    res.status(200).json(new ApiResponse(200, null, 'Role deleted permanently'))
  } catch (error) {
    next(new ApiError(500, 'Internal server error', [error.message]))
  }
})
